...
namespace Engine 
{
	namespace Objects {
		...
		public:
			...
			bool canView(sf::Vector2f scene_start, sf::Vector2f scene_end) const override;
			bool inObject(Math::vec2f); // VirtualDisplay | Scene | Входит ли точка в объект на виртуальной сцене
			bool pointIsObject(sf::Vector2i); // VirtualPosition | Является ли виртуальная точка сцены размещённым объектом.
